...
%  cd("C:/erlang/"). c(ricochetAqueue). c(printData). printData:compare([alevel,aqueue],"boardSimple.txt",red).
...
comparePrint([{SearchType,NumPro}|Rest],BoardFile,Color,FileName) ->
	...
	file:write_file(FileName, io_lib:fwrite("~p\t~p\t~p\t~p\t~p\n", [SearchType,Proc,Time,Nodes,PathCount]),[append]),
...
compareCores(SearchTypes,[FirstCoreCount|Rest],BoardFile,Color,FileName) ->
	...
	file:write_file(FileName, io_lib:fwrite("Cores :~p: ~n", [FirstCoreCount]),[append]),
	comparePrint(SearchTypes,BoardFile,Color,FileName),
